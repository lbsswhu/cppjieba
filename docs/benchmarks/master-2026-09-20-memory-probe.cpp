// Linux/glibc diagnostic only. Compile the same source against each revision.
#include <fstream>
#include <iostream>
#include <iomanip>
#include <malloc.h>
...
int main(int argc, char** argv) {
  const std::string d = (argc > 2 ? std::string(argv[2]) : std::string(".")) + "/dict/";
#ifndef CPPJIEBA_CPU_BASELINE
...
#else
  cppjieba::Jieba jieba(d + "jieba.dict.utf8", d + "hmm_model.utf8", d + "user.dict.utf8", d + "idf.utf8", d + "stop_words.utf8");
#endif
